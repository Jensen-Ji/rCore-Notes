...
对于普通APP，的确依赖操作系统
但是对于操作系统，依赖的是裸机硬件，逻辑上的执行环境是SBI。
...
SBI的功能：完成基本的硬件初始化，跳转到内核
内核：内核的一段是entry.asm，是进行执行前的初始化工作
...
对于内核，RustSBI就是其执行环境，其作用有：
- 在计算机启动时，RustSBI对内核进行初始化，并将控制权移交给内核
- 在内核运行时响应内核的请求，并为其提供服务。

![ch1-003](ch1-003.md)

在 RISC-V 架构中，OS 运行在 S 模式 (Supervisor Mode)
而直接控制电源、时钟等底层硬件的权限通常在更高的 M 模式 (Machine Mode)
SBI就像是 M 模式给 S 模式提供的一组“API 接口”
rCore在S模式，RustSBI在M模式

为什么需要SBI？
写普通程序时，需要打印直接用`printf`或`println!`，因为操作系统处理了繁琐的硬件操作。
写OS时，我就是那个“管理者”。想关机或打印字符，不能直接拔电源或写显存，必须“请求”SBI 帮我做。
